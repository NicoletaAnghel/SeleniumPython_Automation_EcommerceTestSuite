...
class Header(HeaderLocator):
    ...
    def wait_until_cart_item_count(self, count, timeout):
        expected_text = str(count) + ' items'
        self.sl.wait_until_element_contains_text(self.NO_ITEMS, expected_text,timeout)

    def wait_until_cart_1item_count(self, count,timeout):
